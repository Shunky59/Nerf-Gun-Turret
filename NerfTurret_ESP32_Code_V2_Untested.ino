...
unsigned long lastRumbleTime = 0; // Cooldown timer for the rumble

void setup() {
  Serial.begin(115200); // PC Debug (TX0/RX0)
  Serial2.begin(115200, SERIAL_8N1, 16, 17); // Arduino Communication (TX2/RX2)
...
void loop() {
  ...
  if (xboxController.isConnected()) {
    ...
    // ---------------------------------------------------------
    // NEW: Listen for Rumble Commands from Mega
    // ---------------------------------------------------------
    while (Serial2.available() > 0) {
      char inChar = Serial2.read();
      
      // If we receive an 'R' from the Mega, trigger the rumble
      if (inChar == 'R') {
        // Use a 300ms cooldown so we don't spam the controller with BLE packets
        if (millis() - lastRumbleTime > 300) {
          XboxSeriesXHIDReportBuilder_asukiaaa::ReportBase repo;
          repo.v.select.center = true; // Use the main rumble motor
          repo.v.power.center = 60;    // 60% power (adjust between 0-100)
          repo.v.timeActive = 25;      // 25 * 10ms = 250ms rumble duration
          
          xboxController.writeHIDReport(repo);
          lastRumbleTime = millis();
          
          Serial.println("Limit Switch Hit! Controller Rumbling.");
        }
      }
    }
